...
\subsection{Why Fully Connected Networks Fail on Images}
...
\subsection{Convolution in One Dimension}
...
over to two dimensions unchanged.

\paragraph{The operation.}
Let the input be a vector $x = (x_1,\dots,x_D)$. Each output $z_i$ is a weighted
...
\paragraph{Borders: zero padding or valid convolution.}
At the first and last position, Eq.~\eqref{eq:conv1d} needs inputs $x_0$ and
...
A convolutional layer turns the convolution into a network layer in the same way
as in Section~\ref{sec:neural-networks}: add a bias $\beta$ and pass the result
...
\paragraph{A special case of a fully connected layer.}
A fully connected layer computes $h = a[\beta + \Omega x]$ with a full weight
...
\subsection{Channels and Parameter Counts}
...
A hidden unit in the first layer sees $K$ inputs. A unit in the second layer sees
...
\subsection{Two-Dimensional Convolution, Downsampling, and Upsampling}
...
\paragraph{$1\times1$ convolution.}
A kernel of spatial size $1\times1$ looks at a single position but at all $C_i$
...
\subsection{Why Convolution Helps: The MNIST-1D Experiment}
...
\paragraph{The convolutional network.}
The network has three convolutional layers and one fully connected layer,
...
\paragraph{The fully connected comparison.}
A fully connected network with exactly the same number of layers and hidden
...
Image classification is a multiclass problem: the network receives an image and
...
\paragraph{The basic layout of a ConvNet.}
...
with the number of channels doubling after each pooling step. As shown in the
subsection on receptive fields, stacks of small kernels reach large receptive
